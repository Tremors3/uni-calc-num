...
2. La prima somma introduce un nuovo errore $\varepsilon_1$ dovuto all’operazione stessa:
    ...
    >
    > In analisi dell’errore numerico, si **trascurano i termini di secondo ordine**, perché non contribuiscono significativamente alla stima dell’errore relativo complessivo. Questo semplifica il raccoglimento e permette di scrivere i termini come una somma pesata lineare degli errori.
...
+
\underbrace{\sum_{k=1}^{n-1} \frac{\sum_{i=1}^k x_i}{\sum_{i=1}^n x_i} \varepsilon_k  + \varepsilon_n}_{\text{errore dell'algoritmo}}
$$
